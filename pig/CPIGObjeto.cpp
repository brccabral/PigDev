...
void CPIGObjeto::DesenhaBB()
{
    for (int i = 0; i < 4; i++)
    {
        CPIGGerenciadorJanelas::GetJanela(idJanela)->DesenhaLinhaSimples(
                bb[i].x, bb[i].y, bb[(i + 1) % 4].x, bb[(i + 1) % 4].y, VERDE);
    }
    CPIGGerenciadorJanelas::GetJanela(idJanela)->DesenhaRetanguloVazado(
            pos.x, pos.y, alt, larg, VERMELHO);
}

void CPIGObjeto::DesenhaPoligono(PIGCor cor)
{
    int qtdVertices = vertices.size();
    for (int i = 0; i < qtdVertices; i++)
    {
        CPIGGerenciadorJanelas::GetJanela(idJanela)->DesenhaRetangulo(
                vertices[i].x, vertices[i].y, 10, 10, cor);
        CPIGGerenciadorJanelas::GetJanela(idJanela)->DesenhaLinhaSimples(
                vertices[i].x, vertices[i].y, vertices[(i + 1) % qtdVertices].x,
                vertices[(i + 1) % qtdVertices].y, cor);
    }
}

void CPIGObjeto::DesenhaCircular(PIGCor cor) const
{
    SDL_SetRenderDrawColor(renderer, cor.r, cor.g, cor.b, 255);

    SDL_RenderDrawLine(
            renderer, pos.x + pivoAbs.x - raio, altJanela - (pos.y + pivoAbs.y), pos.x + pivoAbs.x,
            altJanela - (pos.y + pivoAbs.y + raio));
    SDL_RenderDrawLine(
            renderer, pos.x + pivoAbs.x, altJanela - (pos.y + pivoAbs.y + raio),
...
void CPIGObjeto::AtualizaBB()
{
    PIGPonto2D pivoAux = {pivoAbs.x + pos.x, -pivoAbs.y + pos.y + alt};

    // SDL_SetRenderDrawColor(renderer, 255, 0, 0, 0);
    // pivoAux.x = pivoRelativo.x + pos.x;
    // pivoAux.y = -pivoRelativo.y + pos.y + alt; //inverte o eixo Y, pois o pivoRel considera o
    // eixo Y aumentando para baixo
...
void CPIGObjeto::AtualizaVertices()
{
...
bool CPIGObjeto::ColisaoCirculoPoligono(vector<PIGPonto2D> vertices) const
{
    ...
    do
    {
        ...
        float mPoligono =
                ((float) vertices[f].y - vertices[i].y) / ((float) vertices[f].x - vertices[i].x);

        if (mPoligono == 0 && PIGValorEntre((pos.x + pivoAbs.x), vertices[i].x, vertices[f].x) &&
            PIGDistancia(
                    {(pos.x + pivoAbs.x), vertices[i].y},
                    {(pos.x + pivoAbs.x), (pos.y + pivoAbs.y)}) <= raio)
        {
            return true;
        }
        if (std::isinf(mPoligono) &&
            PIGValorEntre((pos.y + pivoAbs.y), vertices[i].y, vertices[f].y) &&
            PIGDistancia(
                    {vertices[i].x, (pos.y + pivoAbs.y)},
                    {(pos.x + pivoAbs.x), (pos.y + pivoAbs.y)}) <= raio)
        {
            return true;
        }
        ...
        double xProxCentro =
                ((mPoligono * (vertices[i].x)) - (mCirculo * ((double) pos.x + pivoAbs.x)) +
                 ((double) pos.y + pivoAbs.y) - (vertices[i].y)) /
                (mPoligono - mCirculo);
        double yProxCentro = (mPoligono * (xProxCentro - (vertices[i].x))) + (vertices[i].y);

        if (PIGDistancia({pos.x + pivoAbs.x, pos.y + pivoAbs.y}, {xProxCentro, yProxCentro}) <=
                    raio &&
            PIGValorEntre((int) xProxCentro, vertices[i].x, vertices[f].x))
        {
            return true;
        }
...
CPIGObjeto::CPIGObjeto(
        int idObjeto, const string &nomeArquivo, const int retiraFundo, const PIGCor *corFundo,
        int janela)
...
CPIGObjeto::CPIGObjeto(
        int idObjeto, PIGOffscreenRenderer offRender, int retiraFundo, const PIGCor *corFundo,
        int janela)
...
CPIGObjeto::CPIGObjeto(
        int idObjeto, const CPIGObjeto *objBase, int retiraFundo, const PIGCor *corFundo,
        int janela)
    : CPIGSprite(idObjeto, objBase, retiraFundo, corFundo, janela)
{
    ...
    CPIGObjeto::Move(pos.x, pos.y);
    CPIGObjeto::SetAngulo(angulo);
    AtualizaBB();

    bbAlterado = true;
}

CPIGObjeto::~CPIGObjeto() = default;

void CPIGObjeto::SetVertices(const vector<PIGPonto2D> &verts)
{
    vertices = verts;
    verticesOriginais = verts;
}

void CPIGObjeto::SetAngulo(double a)
{
    CPIGSprite::SetAngulo(a);
    bbAlterado = true;
}

void CPIGObjeto::SetRaioColisaoCircular(int raio)
{
    this->raio = raio;
}

void CPIGObjeto::SetModoColisao(PIGModoColisao valor)
{
    modo = valor;
}

PIGPonto2D CPIGObjeto::GetBB(int i) const
{
    return bb[i];
}

vector<PIGPonto2D> CPIGObjeto::GetVertices()
{
    return vertices;
}

int CPIGObjeto::GetRaio() const
{
    return raio;
}

PIGModoColisao CPIGObjeto::GetModoColisao() const
{
    return modo;
}

void CPIGObjeto::Move(double nx, double ny)
{
    CPIGSprite::Move(nx, ny);
    bbAlterado = true;
}

void CPIGObjeto::Desloca(double dx, double dy)
{
    CPIGSprite::Desloca(dx, dy);
    bbAlterado = true;
}

int CPIGObjeto::Desenha()
{
    CPIGSprite::Desenha();
    /*switch(modo) {//modo
...
bool CPIGObjeto::Colisao(CPIGObjeto *outro)
{
    if (modo == PIG_COLISAO_NENHUMA || outro == NULL)
    {
        return false;
    }
    ...
    if (modo == PIG_COLISAO_OOBB)
    {
        if (modoOutro == PIG_COLISAO_OOBB)
        {
            ...
            {
                // fprintf(arqP,"%f,%f,%d,%d
                // %f,%f,%d,%d\n",pos.x,pos.y,alt,larg,outro->pos.x,outro->pos.y,outro->alt,outro->larg);
                return true; // ColisaoPoligono(outro->GetVertices()) ||
                             // outro->ColisaoPoligono({bb[0], bb[1], bb[2], bb[3]});
            }
        }
        else if (modoOutro == PIG_COLISAO_POLIGONO)
        {
            if (ColisaoOOBB(outro) && outro->ColisaoOOBB(this))
            {
                return ColisaoPoligono(outro->GetVertices()) ||
                       outro->ColisaoPoligono({bb[0], bb[1], bb[2], bb[3]});
            }
        }
    ...
    else if (modo == PIG_COLISAO_POLIGONO)
    {
        if (modoOutro == PIG_COLISAO_POLIGONO)
        {
            ...
            {
                return ColisaoPoligono(outro->GetVertices()) ||
                       outro->ColisaoPoligono(this->vertices);
            }
        }
        else if (modoOutro == PIG_COLISAO_OOBB)
        {
            if (ColisaoOOBB(outro) && outro->ColisaoOOBB(this))
            {
                vector<PIGPonto2D> verticesOutro = {
                        outro->GetBB(0), outro->GetBB(1), outro->GetBB(2), outro->GetBB(3)};
...
bool CPIGObjeto::PontoDentro(PIGPonto2D p)
{
    int i = 0;
...
bool CPIGObjeto::Intersecao(
        const int i, const int f, const vector<PIGPonto2D> &v, const PIGPonto2D p)
{
    const double m = ((double) v[f].y - v[i].y) / ((double) v[f].x - v[i].x);
    int x;

    if (m == 0)
    {
        return p.y == v[i].y && PIGValorEntre(p.x, v[i].x, v[f].x);
    }

    if (p.y == v[i].y && p.x <= v[i].x)
    {
        const int anterior = i == 0 ? v.size() - 1 : i - 1;
    ...
    {
        x = (((double) p.y - v[i].y) / (double) m) + ((double) v[i].x);
    }

    return (x >= p.x) && PIGValorEntre(p.y, v[i].y, v[f].y);
}

bool CPIGObjeto::ColisaoCircular(CPIGObjeto *outro) const
{
    const PIGPonto2D outroPivo = outro->GetPivo();
    const PIGPonto2D outroPos = outro->GetXY();

    return PIGDistancia(
                   {outroPos.x + outroPivo.x, outroPos.y + outroPivo.y},
                   {pos.x + pivoAbs.x, pos.y + pivoAbs.y}) <= (float) (raio + outro->GetRaio());
}

bool CPIGObjeto::ColisaoOOBB(const CPIGObjeto *outro)
{

    // caso em q o angulo é 0
    // caso em que o vetor que vai de bb[0] para bb[1] é paralelo ao eixo X, ou seja, nao ira
    // existir uma "projecao" no eixo X os pontos sao trocados para que seja usado o antigo bb[3]
    // para bb[0]
    // bool swapei = false;
    // string str;
    // char cstr[100]="";
    ...
    double projecaoAB =
            PIGProjecaoX((bb[1].y - bb[0].y) / (bb[1].x - bb[0].x), bb[0]); //,swapei,vet);
    double projecaoCD =
            PIGProjecaoX((bb[3].y - bb[2].y) / (bb[3].x - bb[2].x), bb[2]); //,swapei,vet);
    ...
    for (int i = 0; i < 4; i++)
    {
        projecao[i] = PIGProjecaoX(
                (bb[1].y - bb[0].y) / (bb[1].x - bb[0].x),
                outro->GetBB(i)); //,swapei,vet);
    }
    ...
    projecaoAB = PIGProjecaoY((bb[2].y - bb[1].y) / (bb[2].x - bb[1].x), bb[1]);
    projecaoCD = PIGProjecaoY((bb[0].y - bb[3].y) / (bb[0].x - bb[3].x), bb[3]);

    // projecao dos pontos(com o angulo dos anteriores) no eixo Y do outro objeto
    for (int i = 0; i < 4; i++)
    {
        projecao[i] = PIGProjecaoY((bb[2].y - bb[1].y) / (bb[2].x - bb[1].x), outro->GetBB(i));
    }
    ...
    (%d,%d)\n",outro->GetBB(0).x,outro->GetBB(0).y,outro->GetBB(1).x,outro->GetBB(1).y,outro->GetBB(2).x,outro->GetBB(2).y,outro->GetBB(3).x,outro->GetBB(3).y);
                vet.push_back(cstr);
                sprintf(cstr,"%f %f %f %f proj %f
    %f\n",menorA,maiorB,maiorA,menorB,projecaoAB,projecaoCD); vet.push_back(cstr);
...
bool CPIGObjeto::ColisaoPoligono(const vector<PIGPonto2D> &vertices)
{
    for (auto vertice: vertices)
    {
        if (PontoDentro(vertice))
        {
            return true;
        }
    }
