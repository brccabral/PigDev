...
/********************************
O tipo PIGCor é um struct formado por 4 campos: r,g,b,a correspondendo aos valores de vermelho,
verde, azul e alfa (transparência) do pixel
********************************/
#if _MSC_VER
typedef __declspec(deprecated(
        "PIG_Cor nao deve mais ser usado. Utilize PIGCor.")) SDL_Color PIG_Cor;
#else
[[deprecated("PIG_Cor nao deve mais ser usado. Utilize PIGCor.")]] typedef SDL_Color PIG_Cor;
#endif
...
#define TRANSPARENTE ((SDL_Color){0, 0, 0, 0})
...
PIG_EVENTO_JANELA: indica algum possível evento relacionado à janela de jogo (redimensionamento,
movimento, perda ou ganho de foco etc) PIG_EVENTO_TECLADO: indica algum possível evento relacionado
ao teclado do computador (tecla pressionada ou liberada, texto digitado ou editado etc)
PIG_EVENTO_MOUSE: indica algum posspivel evento relacionado ao mouse do compuatdor (botão
pressionado ou liberado, movimentação, uso da rodinha etc) PIG_EVENTO_AUDIO: indica o final da
execução de algum tipo de audio (efeito ou background) PIG_EVENTO_CONTROLADOR: indica algum possível
evento relacionado aos botões ou eixos do controlador PIG_EVENTO_USUARIO: (não implementado)
********************************/
typedef enum PIGTipoEvento
{
    PIG_EVENTO_NULO,
    PIG_EVENTO_GERAL,
    PIG_EVENTO_JANELA,
...
acao: código que representa o evento específico (os possíveis valores podem ser vistos abaixo do
struct) posX: a posição no eixo X (nas coordenadas da tela) do ponteiro do mouse no momento de
ocorrência do evento posY: a posição no eixo Y (nas coordenadas do tela) do ponteiro do mouse no
momento de ocorrência do evento worldX: a posição no eixo X (nas coordenadas do mundo) do ponteiro
do mouse no momento de ocorrência do evento worldY: a posição no eixo Y (nas coordenadas do mundo)
do ponteiro do mouse no momento de ocorrência do evento botao: indica qual dos possíveis três botões
foi pressionado ou liberado cliques: indica a quantidade de cliques a que se refere o evento
(1=clique simples, 2=clique duplo, ...) relX: indica o deslocamento no eixo X quando há uma
movimentação de mouse; adicionalmente, representa o deslocamento da rodinha no eixo X (não usado)
relX: indica o deslocamento no eixo Y quando há uma movimentação de mouse; adicionalmente,
representa o deslocamento da rodinha no eixo Y
********************************/
...
#define PIG_MOUSE_X1 SDL_BUTTON_X1
#define PIG_MOUSE_X2 SDL_BUTTON_X2

/********************************
O PIGInfoEventoJanela é um struct contendo informações específicas sobre o evento de janela:
acao: código que representa o evento específico (os possíveis valores podem ser vistos abaixo do
struct) dado1,dado2: indicam em conjunto a informação específica do evento, podendo representar o
ponto (X,Y), em relação ao monitor, para o qual a janela foi movida ou o novo tamanho da janela após
uma alteração de dimensões na mesma
********************************/
typedef struct PIGInfoEventoJanela
{
    SDL_WindowEventID acao;
    int numeroJanela;
    int dado1, dado2;
} PIGInfoEventoJanela;
...
acao: código que representa o evento específico (os possíveis valores podem ser vistos abaixo do
struct) tecla: código que representa a tecla pressionada ou liberada. Os possíveis valores pdoem ser
vistos no final deste arquivo. repeticao: indica se está havendo repetição no acionamento desta
tecla ou não texto: indica o que está sendo inserido inicio: indica a posição virtual da parte
selecionada do texto a ser editado tamanhoSelecao: indica o tamanho (quantidade de caracteres) da
seleção do texto a ser editado
********************************/
...
/********************************
O PIGInfoEventoAudio é um struct contendo informações específicas sobre o evento de audio
(encerramento de audio): audioId: id do audio que se encerrou (se o valor for igual a
PIG_AUDIO_BACKGROUND, indica que o audio de background se encerrou)
********************************/
...
/********************************
O PIGInfoEventoControlador é um struct contendo informações específicas sobre o evento de
controlador (joystick): acao: código que representa o evento específico (os possíveis valores podem
ser vistos abaixo do struct) eixo: qual dos eixos digital ou analágico, vertical ou horizontal) foi
acionado valor: botao: qual dos botões digitais foi acionado pecentualEixo: qual o valor percentual
do eixo acionado idControlador: qual dos controladores gerou o evento
********************************/
...
typedef enum PIGTipoMensagemRede
{
    PIG_REDE_CONEXAO,
...
#if _MSC_VER
typedef __declspec(deprecated(
        "PIG_Evento nao deve mais ser usado. Utilize PIGEvento.")) PIGEvento PIG_Evento;
#else
[[deprecated(
        "PIG_Evento nao deve mais ser usado. Utilize PIGEvento.")]] typedef PIGEvento PIG_Evento;
#endif
...
#if _MSC_VER
typedef __declspec(deprecated(
        "PIG_Teclado nao deve mais ser usado. Utilize PIGTeclado.")) PIGTeclado PIG_Teclado;
#else
[[deprecated("PIG_Teclado nao deve mais ser usado. Utilize PIGTeclado.")]] typedef PIGTeclado
        PIG_Teclado;
#endif
...
#if _MSC_VER
typedef __declspec(deprecated(
        "PIG_Flip nao deve mais ser usado. Utilize PIGFlip.")) PIGFlip PIG_Flip;
#else
[[deprecated("PIG_Flip nao deve mais ser usado. Utilize PIGFlip.")]] typedef PIGFlip PIG_Flip;
#endif
...
#if _MSC_VER
typedef __declspec(deprecated(
        "PIG_Estilo nao deve mais ser usado. Utilize PIGEvento.")) int PIG_Estilo;
#else
[[deprecated("PIG_Estilo nao deve mais ser usado. Utilize PIGEvento.")]] typedef int PIG_Estilo;
#endif
typedef int PIGEstilo;
#define PIG_TOTALESTILOS 16
#define PIG_FONTE_PADRAO_NOME "assets/fontes/MesloLGS NF Regular.ttf"
...
typedef struct PIGMetricasFonte
{
    ...
    int descent; // quantidade de pixels abaixo da linha horizontal de base da letra (sempre
                 // positivo)
    int altura; // total de pixels na altura (soma dos dois campos anteriores)
    int recuo; // quantidade pixels entre a linha vertical de base e o primeiro pixel do eixo X da
               // letra (pode ser negativo)
    int avanco; // quantidade de pixels entre a linha vertical de base e o último pixel do eixo X da
                // letra (sempre positivo)
