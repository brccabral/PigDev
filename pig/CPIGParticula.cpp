...
double CPIGParticula::ModificaHP(const double valor)
{
    hp += valor;
    viva = hp > 0;
    return hp;
...
CPIGParticula::CPIGParticula(
        int idParticula, PIGAnimacao base, int vida, int retiraFundo, const PIGCor *corFundo,
        int idJanela)
...
void CPIGParticula::DefineLimites(SDL_Rect espacoMax, double tempoMax)
{
...
bool CPIGParticula::ChecaViva()
{
    // printf("tempo %f <
    // %f\n",CPIGGerenciadorTimers::GetTimer(timer)->GetTempoDecorrido(),tempoVida); printf("viva
    // antes %d\n",viva);
    viva = viva && CPIGGerenciadorTimers::GetTimer(timer)->GetTempoDecorrido() <= tempoVida &&
           (pos.x > espacoVida.x) && (pos.x < espacoVida.w) && (pos.y > espacoVida.y) &&
           (pos.y < espacoVida.h);
...
bool CPIGParticula::Colisao(CPIGParticula *outra)
{
