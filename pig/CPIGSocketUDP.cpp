...
void CPIGSocketUDP::CriaEventoMensagem(
        PIGTipoMensagemRede tipoMensagem, const UDPpacket *pacoteRecebido)
{
...
CPIGSocketUDP::CPIGSocketUDP(int idSocket, int porta)
{
    id = idSocket;
    portaLocal = porta;
    thread = NULL;
    ...
    (void) sprintf(
            hostLocal, "%d.%d.%d.%d", (int) part[0], (int) part[1], (int) part[2], (int) part[3]);
...
bool CPIGSocketUDP::EnviaDados(const void *buffer, int tamanhoBuffer, const string &host, int porta)
{
    if (!ativo)
    {
        return false;
    }
    if (tamanhoBuffer > PIG_TAMANHO_PACOTE_UDP)
    {
        return false;
    }
...
[[nodiscard]] int CPIGSocketUDP::GetId() const
{
    return id;
}

[[nodiscard]] bool CPIGSocketUDP::GetAtivo() const
{
    return ativo;
}

[[nodiscard]] int CPIGSocketUDP::GetPortaLocal() const
{
    return portaLocal;
}

string CPIGSocketUDP::GetHostLocal()
{
    return hostLocal;
}

[[nodiscard]] long CPIGSocketUDP::GetBytesEnviados() const
{
    return qtdBytesEnviados;
}

[[nodiscard]] long CPIGSocketUDP::GetBytesRecebidos() const
{
    return qtdBytesRecebidos;
}

[[nodiscard]] long CPIGSocketUDP::GetPacotesEnviados() const
{
    return qtdPacotesEnviados;
}

[[nodiscard]] long CPIGSocketUDP::GetPacotesRecebidos() const
{
    return qtdPacotesRecebidos;
}

[[nodiscard]] long CPIGSocketUDP::GetTempoUltimoRecebido() const
{
    return tempoPacoteRecebido;
}

[[nodiscard]] long CPIGSocketUDP::GetTempoUltimoEnviado() const
{
    return tempoPacoteEnviado;
}

int CPIGSocketUDP::thread_code(void *data)
{
