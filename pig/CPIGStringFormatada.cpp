...
int CPIGStringFormatada::GetIntLetra(int indice)
{
    return letras[indice];
}

PIGCor CPIGStringFormatada::GetCor(int indice)
{
    return cores[indice];
}

PIGEstilo CPIGStringFormatada::GetEstilo(int indice)
{
...
void CPIGStringFormatada::Adiciona(int letra, int larguraAcumulada, PIGCor cor, PIGEstilo estilo)
{
...
vector<CPIGStringFormatada> CPIGStringFormatada::SeparaPalavras(const string &delim)
{
...
vector<CPIGStringFormatada> CPIGStringFormatada::ExtraiLinhas(int largMax, const string &delim)
{
    vector<CPIGStringFormatada> resp;
    if (letras.empty())
    {
    ...
    CPIGStringFormatada linhaAtual = palavras[0]; // linha atual (que está sendo montada) contém
                                                  // pelo menos a primeira palavra

    for (unsigned int i = 1; i < palavras.size(); i++)
    {
        CPIGStringFormatada &palavra = palavras[i]; // pega a próxima palavra

        if (CPIGStringFormatada::LargTotalPixels() + CPIGStringFormatada::LargTotalPixels() >
            largMax)
        ...
        { // se existe uma quebra de linha forçada
            ...
            {
                linhaAtual.letras.clear();
            }
        }
    }

    if (!linhaAtual.letras.empty())
    {
...
CPIGStringFormatada CPIGStringFormatada::operator+=(const CPIGStringFormatada &outra)
{
    int largAtual = LargTotalPixels();
    for (unsigned int i = 0; i < outra.letras.size(); i++)
    {
        Adiciona(
                outra.letras[i], outra.largAcumulada[i] + largAtual, outra.cores[i],
                outra.estilos[i]);
    }
    return *this;
}
