...
CPIGStringFormatada CPIGFonteDinamica::Processa(const string &textoOrig, PIGCor corInicial)
{
    ...
    int estado = 0; // estado atual da máquina de estados
    unsigned int i = 0;
    ...
    while (i < letras.size())
    {
        ...
        switch (estado)
        {
            case 0: // estado sem formatacao
                if (letra == '<')
                { // alerta de entrada de formatacao
                    estado = 1;
                }
                else if (letra == '@')
                { // alerta de saída de cor
                    estado = 3;
                }
                else if (
                        letra == PIG_SIMBOLO_NEGRITO || letra == PIG_SIMBOLO_ITALICO ||
                        letra == PIG_SIMBOLO_SUBLINHADO || letra == PIG_SIMBOLO_CORTADO)
                { // alerta de saída de negrito, itálico, underline ou strike
                    estado = 4;
                }
                else
                {
            ...
            case 1: // alerta para inicio de formatacao
                if (letra == '@')
                { // realmente é entrada de cor
                    estado = 2;
                }
                else if (letra == PIG_SIMBOLO_NEGRITO)
                { // negrito
                ...
                { // năo é entrada de cor nem de formatacao de estilo
                    if (letraAnt > 0)
                    {
                        larguraTotal += larguraLetra[estilo][letraAnt - PIG_PRIMEIRO_CAR];
                        resp.Adiciona(
                                letraAnt, larguraTotal, cor, estilo); // devolve o token anterior
                    }
            ...
            case 2: // reconhecimento da cor
                char stringCor[11];
                strncpy(stringCor, &textoOrig[i],
                        10); // pega os caracteres em hexadecimal (ex: 0xffa64312)
                stringCor[10] = '\0';
            ...
            case 3: // alerta para saida de cor
                if (letra == '>')
                { // realmente saída da cor
                    pilhaCor.pop_back();
                    if (!pilhaCor.empty())
                    { // tira a cor da pilha e pega a de baixo
            ...
            case 4: // saida de negrito, itálico, underline ou strike
                if (letra == '>')
                {
                    ...
                    if (pilhaEstilo.size() > 1)
                    { // tira a cor da pilha e pega a de baixo
                        estilo = pilhaEstilo[pilhaEstilo.size() - 1];
                    }
                    else
                    {
                        estilo = PIG_ESTILO_NORMAL;
                    }
                }
                else
                { // năo é saída de cor
...
CPIGFonteDinamica::CPIGFonteDinamica(const string &nomeFonte, int tamanhoFonte, int janela)
...
void CPIGFonteDinamica::Escreve(
        CPIGStringFormatada formatada, int x, int y, PIGPosTexto pos, float ang, int alvoTextura)
{
    int delta = 0;
    switch (pos)
    {
        ...
        case PIG_TEXTO_DIREITA:
            x -= formatada.LargTotalPixels();
            delta = formatada.LargTotalPixels();
            break;
        case PIG_TEXTO_CENTRO:
            x -= formatada.LargTotalPixels() / 2;
            delta = formatada.LargTotalPixels() / 2;
            break;
    }
    ...
    for (int i = 0; i < formatada.size(); i++)
    {
        ...
        {
            SDL_SetTextureBlendMode(glyphsT[estiloAtual][indice], SDL_BLENDMODE_BLEND);
        }
        ...
        SDL_RenderCopyEx(
                render, glyphsT[estiloAtual][indice], NULL, &rectDestino, -ang, &ponto,
                PIG_FLIP_NENHUM);
...
void CPIGFonteDinamica::Escreve(const string &texto, int x, int y)
{
    Escreve(texto, x, y, BRANCO, PIG_TEXTO_ESQUERDA, 0, 0);
}
// escreve uma string normal, alinha com o ponto x,y e o parâmetro pos
void CPIGFonteDinamica::Escreve(const string &texto, int x, int y, PIGCor corFonte, PIGPosTexto pos)
{
...
void CPIGFonteDinamica::Escreve(const string &texto, SDL_Texture *textura, PIGCor cor)
{
...
// escreve uma string longa (múltiplas linhas), incluindo formataçăo interna, alinhada de acordo com
// o ponto x,y e o parâmetro pos
void CPIGFonteDinamica::EscreveLonga(
        const string &texto, int x, int y, int largMax, int espacoEntreLinhas)
{
...
// escreve uma string longa (múltiplas linhas), com formataçăo já processada e fornecida, alinhada
// de acordo com o ponto x,y e o parâmetro pos
void CPIGFonteDinamica::EscreveLonga(
        const vector<CPIGStringFormatada> &linhas, int x, int y, int espacoEntreLinhas)
{
    EscreveLonga(linhas, x, y, espacoEntreLinhas, PIG_TEXTO_ESQUERDA, 0);
}
...
{
    int yTotal = y;
    for (const CPIGStringFormatada &str: linhas)
    {
...
int CPIGFonteDinamica::GetLarguraPixelsString(const string &texto)
{
    if (texto.empty())
    {
        return 0;
...
vector<CPIGStringFormatada> CPIGFonteDinamica::ExtraiLinhas(const string &texto, int largMax)
{
    CPIGStringFormatada formatada = Processa(texto);
    vector<CPIGStringFormatada> linhas = formatada.ExtraiLinhas(largMax, delimitadores);
    formatada.Clear();
